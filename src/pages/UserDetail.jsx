...
export default function UserDetail({ user, onBack, onKycStatusChange, authToken }) {
  ...
  useEffect(() => {
    const fetchKycData = async () => {
      setLoadingKyc(true)
      try {
        const response = await fetch(`https://macstormbattle-backend.onrender.com/api/userkyc/user/${userId}`, {
          method: "GET",
          headers: {
  ...
  const fetchPaymentData = async () => {
    ...
    try {
      const depositResponse = await fetch(`https://macstormbattle-backend.onrender.com/api/user/deposit/${userId}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${authToken}`,
      ...
      const withdrawalResponse = await fetch(
        `https://macstormbattle-backend.onrender.com/api/user/withdraw/${userId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const fetchTransactionsData = async () => {
    ...
    try {
      const response = await fetch(
        `https://macstormbattle-backend.onrender.com/api/match/match-transaction-logs/user/${userId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
          ...
          >
            ...
            <table className="min-w-full text-sm border dark:border-zinc-700 rounded-lg overflow-hidden">
              <tbody className="divide-y dark:divide-zinc-700">
                ...
                <tr className="bg-gray-50 dark:bg-zinc-700">
                  <td className="font-medium p-2">Diamonds</td>
      ...
      case "Diamond Info":
        return (
          <div className="space-y-6">
            {loadingPayments ? (
              <div className="flex justify-center items-center h-32">
                <p className="text-sm text-gray-500">Loading Diamond info...</p>
              </div>
            ) : (
              <>
                {/* Summary Cards */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                  <div className="border rounded-lg p-4 dark:border-zinc-700 bg-green-50 dark:bg-green-900/20">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm text-green-600 dark:text-green-400">Total Diamonds</p>
                        <p className="text-2xl font-bold text-green-700 dark:text-green-300">
                          💎{depositData.reduce((sum, deposit) => sum + Number.parseFloat(deposit.amount), 0).toFixed(2)}
                        </p>
                        <p className="text-xs text-green-600 dark:text-green-400">{depositData.length} times</p>
                      </div>
                  ...
                  <div className="border rounded-lg p-4 dark:border-zinc-700 bg-red-50 dark:bg-red-900/20">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm text-red-600 dark:text-red-400">Total Redeem Diamonds</p>
                        ...
                        <p className="text-xs text-red-600 dark:text-red-400">{withdrawalData.length} times</p>
                      </div>
                ...
                <div className="border rounded-lg dark:border-zinc-700">
                  ...
                  {depositData.length > 0 ? (
                    ...
                    <p className="text-gray-500 text-center py-8">No deposit Diamonds found</p>
                  )}
                </div>
                ...
                <div className="border rounded-lg dark:border-zinc-700">
                  ...
                  {withdrawalData.length > 0 ? (
                    ...
                    <p className="text-gray-500 text-center py-8">No Redeem Diamonds found</p>
                  )}
                </div>
              </>
            )}
