...
import axiosInstance from "../utils/axios" // Pre-configured axios instance

// API helper functions using axios
const fetchContests = async () => {
  try {
    const response = await axiosInstance.get('/duo-contests')
    return response.data
  } catch (error) {
    throw new Error(`Failed to fetch contests: ${error.response?.data?.message || error.message}`)
  }
}

const fetchTeams = async (contestId) => {
  try {
    const response = await axiosInstance.get(`/duo-contests/${contestId}/teams`)
    return response.data
  } catch (error) {
...
const deletePlayer = async (contestId, teamNumber, playerNumber) => {
  try {
    const response = await axiosInstance.delete(`/duo-contests/${contestId}/join/${teamNumber}/${playerNumber}`)
    return response.data
  } catch (error) {
    throw new Error(`Failed to delete player: ${error.response?.data?.message || error.message}`)
  }
}

const updateRoom = async (contestId, roomData) => {
  try {
    const response = await axiosInstance.put(`/duo-contests/${contestId}/room`, roomData)
    return response.data
  } catch (error) {
...
function ContestDetail({ contest, onBack }) {
  ...
  const handleSaveRoom = async () => {
    try {
      ...
      alert(`Failed to update room details: ${err.message}`)
    } finally {
      setSavingRoom(false)
    }
  }
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-sm font-medium text-blue-600">{contest.winCriteria}</div>
                  </div>
                </div>
              </button>
            ))
