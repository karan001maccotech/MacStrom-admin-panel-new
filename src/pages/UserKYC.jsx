...
export default function UserKYC() {
  ...
  useEffect(() => {
    const fetchUsersAndTransactions = async () => {
      try {
        ...
        const usersResponse = await axiosInstance.get("/auth/user");
        ...
        const mappedUsersPromises = usersData.users.map(async (apiUser) => {
          ...
          try {
            // Fetch deposits using axios
            const depositResponse = await axiosInstance.get(`/user/deposit/${apiUser.member_id}`);
            if (depositResponse.data.status && depositResponse.data.data) {
              totalDeposits = depositResponse.data.data.reduce(
                (sum, record) => sum + Number(record.amount),
                0
              );
            }

            // Fetch withdrawals using axios
            const withdrawalResponse = await axiosInstance.get(`/user/withdraw/${apiUser.member_id}`);
            if (withdrawalResponse.data.status && withdrawalResponse.data.data) {
              totalWithdrawals = withdrawalResponse.data.data.reduce(
                (sum, record) => sum + Number(record.amount),
                0
              );
            }
  ...
  const handleBan = async (userId, currentStatus) => {
    ...
    try {
      await axiosInstance.put("/v1/user/member-status/admin/update", {
        user_id: userId,
  ...
  const handleDeleteConfirm = async () => {
    if (deleteUser) {
      try {
        await axiosInstance.delete(`/v1/user/${deleteUser.id}`);
  ...
  if (selectedUser) {
    return (
      <UserDetail
        ...
        axiosInstance={axiosInstance} // Pass axios instance instead of authToken
      />
    );
  }
  ...
  return (
    <div className="p-6 min-h-screen bg-gray-50 dark:bg-zinc-900 text-gray-900 dark:text-white">
            ...
            >
              ...
              <div className="flex items-center justify-between flex-wrap gap-4 mt-4 md:mt-0 text-sm">
                ...
                <div className="flex space-x-2">
                  ...
                  <button onClick={() => handleBan(user.id, user.status)}>
                    <BanIcon
                      className={`w-4 h-4 ${
                        user.status === "Active"
                          ? "text-red-500"
      ...
      {walletUser && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white dark:bg-zinc-800 p-6 rounded shadow-md w-full max-w-md">
            <h2 className="text-lg font-bold mb-1 text-gray-900 dark:text-white">Wallet Adjustment</h2>
            ...
            <div className="grid gap-4">
              ...
              <div>
                <label className="text-sm font-medium text-gray-900 dark:text-white">Admin PIN</label>
                <input
                  type="password"
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button
                ...
                className="px-4 py-2 border rounded dark:border-zinc-600 dark:text-white"
              >
                Cancel
              </button>
              <button
