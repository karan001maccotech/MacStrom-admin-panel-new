...
export default function AdminBonusPanel() {
  ...
  const fetchBonuses = async () => {
    try {
      setLoading(true)
      setError('')
      const response = await axiosInstance.get('/bonus/all')
      setBonuses(response.data)
    } catch (err) {
      const errorMessage = err.response?.data?.message || err.message || 'Unknown error occurred'
      setError('Failed to fetch bonuses: ' + errorMessage)
  ...
  const handleAddBonus = async () => {
    ...
    try {
      ...
      await axiosInstance.post('/bonus/add', {
      ...
      const errorMessage = err.response?.data?.message || err.message || 'Unknown error occurred'
      toast.error('Failed to add bonus: ' + errorMessage)
  ...
  const handleEditSave = async () => {
    ...
    try {
      setActionLoading(true)
      await axiosInstance.put('/bonus/update', {
      ...
      const errorMessage = err.response?.data?.message || err.message || 'Unknown error occurred'
      toast.error('Failed to update bonus: ' + errorMessage)
  ...
  const handleDelete = async (memberId) => {
    ...
    try {
      setActionLoading(true)
      await axiosInstance.delete(`/bonus/delete/${memberId}`)
      ...
      const newTotalPages = Math.ceil(response.data.length / rowsPerPage)
      if (page >= newTotalPages && newTotalPages > 0) {
        setPage(newTotalPages - 1)
      }
    } catch (err) {
      const errorMessage = err.response?.data?.message || err.message || 'Unknown error occurred'
      toast.error('Failed to delete bonus: ' + errorMessage)
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        ...
        <Grid item xs={12} sm={4}>
          <Card sx={{ bgcolor: 'info.main', color: 'white' }}>
            <CardContent>
              <Typography variant="h6">Total Bonus Diamonds</Typography>
            ...
            <Table size="small">
              <TableHead>
                <TableRow sx={{ bgcolor: 'grey.50' }}>
                  ...
                  <TableCell sx={{ fontWeight: 'bold' }}>Bonus Diamonds</TableCell>
