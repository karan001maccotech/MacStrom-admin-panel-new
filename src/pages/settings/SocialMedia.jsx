...
const SocialMedia = () => {
  ...
  useEffect(() => {
    const fetchSocialMediaLinks = async () => {
      try {
        setIsLoading(true)
        const response = await axios.get("https://macstormbattle-backend-2.onrender.com/api/socialmedialinks")
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updatePromises = Object.entries(formData).map(([platform, url]) =>
        axios.put(`https://macstormbattle-backend-2.onrender.com/api/socialmedialinks/${platform}`, {
          url: url.trim(),
          platform: platform
        })
      )
